...
def test(data,
    ...
    for batch_i, (img, depths, poses, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, train_out = model.forward_MV(img, depths, poses, width, augment=augment)  # inference and training outputs
